...
combined and normalized on a per candidate basis. A heatmap of the total feature 
...
function, $f(x) = w^Tx + b$, in a higher dimensional inner product space
defined by a kernel function, $K(x_i,x_j)=\phi (x_i)^T \phi (x_j)$. \\
...
\end{align*}
where $\phi(x)$ is a mapping to the higher dimension space, $C$ determins the
degree of regularization, and $\zeta$, $\zeta^*$ are slack variables which
allow for constraint relaxation in the case it is required. This problem is
often simpler to solve in its dual form:
\begin{equation*}
